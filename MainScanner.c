...
ReaderPointer keywordLiteralTable;
mgn_int errorNumber;				/* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern mgn_int line; /* Source code line numbers - defined in scanner.c */
 ...
 * -------------------------------------------------------------
 */
mgn_void printScannerError(mgn_ch* fmt, ...);
mgn_void displayScanner(BufferReader* ptrBuffer);
mgn_long getScannerFilesize(mgn_ch* fname);
mgn_void printToken(Token currentToken);
...
mgn_int mainScanner(mgn_int argc, mgn_ch** argv) {
	...
	if (getScannerFilesize(argv[2]) == 0) {
		Token t = { 0 };
		t.code = SEOF_T;
		printf("----------------------------------\n");
	...
	keywordLiteralTable = readerCreate(READER_DEFAULT_SIZE, READER_DEFAULT_INCREMENT, MODE_ADDIT);
	if (keywordLiteralTable == NULL) {
		printScannerError("%s%s", argv[0], ": Could not create string literals buffer");
		exit(EXIT_FAILURE);
	}
	...
	printf("Token\currentToken\tAttribute\n");
	printf("----------------------------------\n");
	do {
	...
	readerRestore(keywordLiteralTable);
